...
void fpr2write(int num_of_en) { //write from en
    fopen_s(&fpr2, "engineer.bin", "wb");
    if (fpr2 == NULL)
        printf("error");
    fwrite(&en, sizeof(struct engineer), num_of_en, fpr2);
    fclose(fpr2);
}
void fpw2read(int num_of_en) // read from enineer.bin
{
    //Open the file to read
    fopen_s(&fpw2, "engineer.bin", "rb");
    if (fpw2 == NULL)
        printf("error");
...
void newengineer(int num_of_engineer) {
    //t[0].details = { '0' };
...
void fprwrite(int num_of_user) { // write users. 
...
void newuser(int num_of_user) {
    for (int n = 0; n < num_of_user; n++) {
        ...
        printf("Get the location ");
        scanf_s("%d", &user[n].location);
        printf("Get the productID ");
        scanf_s("%d", &user[n].productID);
        printf("Get the problem ");
